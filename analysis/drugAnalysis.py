    ...
    def convert(self, target):
        ...
        for t in target:
            try:
                ...
                continue
        return r
        # return [self.converter[t] for t in target]

"""if __name__ == '__main__':
    with open('Data/drug_target_UniprotID_all.json') as f:
        drugTarget = json.load(f)
    with open('Data/breast_cancer_drugs_set') as f:
        breastCancerDrugs = json.load(f)
    with open('Data/uniprot_hgnc_converter.json') as f:
        converterFile = json.load(f)
    drugAnalyser = DrugReactionAnalysis(targets=drugTarget,
                                        converter=converterFile) # model='Data/recon2.json', drugs=breastCancerDrugs,

    print(drugAnalyser.drug_target('DB12015'))
