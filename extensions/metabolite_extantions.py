...
def producers(self, without_transports=False):
    reactions = filter(lambda r: r.metabolites[self] > 0 and not r.id.endswith('spill'), self.reactions)
...
def consumers(self):
    return [r for r in self.reactions if r.metabolites[self] < 0 and not r.id.endswith('spill')]


def total_stoichiometry(self, without_transports=False):
    return sum(abs(r.metabolites[self]) for r in self.reactions if not r.id.endswith('spill'))
